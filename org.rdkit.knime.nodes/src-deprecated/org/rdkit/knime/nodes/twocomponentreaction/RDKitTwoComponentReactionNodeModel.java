...
public class RDKitTwoComponentReactionNodeModel extends NodeModel {
	 ...
	 */
	 ...
	 private int[] findColumnIndices(final DataTableSpec[] specs)
			 throws InvalidSettingsException {
		 final String first = m_reactant1Col.getStringValue();
		 if (first == null) {
			 throw new InvalidSettingsException("Not configured yet");
		 }
		 final int firstIndex = specs[0].findColumnIndex(first);
		 if (firstIndex < 0) {
			 throw new InvalidSettingsException(
		 ...
		 final String second = m_reactant2Col.getStringValue();
		 if (second == null) {
			 throw new InvalidSettingsException("Not configured yet");
		 }
		 final int secondIndex = specs[1].findColumnIndex(second);
		 if (secondIndex < 0) {
			 throw new InvalidSettingsException(
	 ...
	 protected BufferedDataTable[] execute(final BufferedDataTable[] inData,
		 ...
		 // 2.
		 final boolean doMatrix = m_doMatrix.getBooleanValue();
		 try {
			 int r1Count = 0;
			 ...
			 while (it1.hasNext()) {
				 final DataRow row1 = it1.next();
				 r1Count++;
				 final DataCell r1Cell = row1.getCell(indices[0]);
				 if (!doMatrix) {
				 ...
				 // doMatrix is false
				 while (it2.hasNext()) {
					 final DataRow row2 = it2.next();
					 r2Count++;
					 final DataCell r2Cell = row2.getCell(indices[1]);
					 if (!r2Cell.isMissing()) {
                      ...
                      mol2 = ((AdapterValue)r2Cell).getAdapter(RDKitMolValue.class).readMoleculeValue();
						 ...
						 if (mol2 != null) {
							 ...
							 // products.
							 final ROMol_Vect_Vect prods = rxn.runReactants(rs);
							 if (!prods.isEmpty()) {
