...
public class DescriptorCalculationNodeModel extends AbstractRDKitCalculatorNodeModel {
		...
		if (outPort == 0) {
			...
			arrOutputFactories[0] = new AbstractRDKitCellFactory(this, AbstractRDKitCellFactory.RowFailurePolicy.DeliverEmptyValues,
				...
				public DataCell[] process(final InputDataInfo[] arrInputDataInfo, final DataRow row, final long lUniqueWaveId) throws Exception {
					final DataCell[] arrAllResults = new DataCell[iNewColumnCount];

					// Calculate the new cells
					final ROMol mol = markForCleanup(arrInputDataInfo[INPUT_COLUMN_MOL].getROMol(row), lUniqueWaveId);

					int iOffset = 0;
					for (final Descriptor descriptor : arrDescriptors) {
						if (descriptor != null) {
							for (final DataCell cell : descriptor.calculate(mol, warningConsolidator)) {
	 ...
	 */
	private DataColumnSpec[] createDescriptorColumnSpecs(final DataTableSpec inSpec) {
		final Descriptor[] arrDescriptors = m_modelDescriptors.getValues();
		final List<DataColumnSpec> listNewColumns = new ArrayList<DataColumnSpec>();

		if (arrDescriptors != null) {
			final List<String> listNewNames = new ArrayList<String>();

			for (final Descriptor descriptor : arrDescriptors) {
				if (descriptor != null) {
					final int iColumnCount = descriptor.getColumnCount();
					final DataType[] arrTypes = descriptor.getDataTypes();
					final String[] arrTitles = descriptor.getPreferredColumnTitles();

					for (int i = 0; i < iColumnCount; i++) {
						final String strUniqueColumnName = SettingsUtils.makeColumnNameUnique(
