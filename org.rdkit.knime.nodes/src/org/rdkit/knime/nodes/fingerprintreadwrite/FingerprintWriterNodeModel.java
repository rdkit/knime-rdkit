...
public class FingerprintWriterNodeModel extends NodeModel {
    ...
    protected DataTableSpec[] configure(final DataTableSpec[] inSpecs)
        ...
        File file = FPSReadWriteUtil.checkFile(m_outputFile, false);
        if (file.isDirectory()) {
        ...
        DataColumnSpec target = in.getColumnSpec(m_fpsColumn);
        if (target == null) {
    ...
    protected BufferedDataTable[] execute(final BufferedDataTable[] inData, 
        ...
        File outFile = FPSReadWriteUtil.checkFile(m_outputFile, false);
        ...
        try {
	        ...
	        for (DataRow r : in) {
	            ...
	            DataCell cell = r.getCell(colIndexFps);
	            //Read the ids either from rowkey or from id column.
	            if(!m_writeRowid && colIndexId != -1) {
	            	StringCell idCell = (StringCell)r.getCell(colIndexId);
	            	id = idCell.getStringValue();
	            } else {
	            ...
	            if (!cell.isMissing()) {
	            	...
	            	DenseBitVectorCell bvc = (DenseBitVectorCell)cell;
	                DenseBitVector bv = bvc.getBitVectorCopy();
	                int len = new Long(bv.length()).intValue();
	                long[] bitArr = new long[len];
	               ...
	               // FPSReadWriteUtil fps = new FPSReadWriteUtil(m_outputFile);
	                String toString = FPSReadWriteUtil.convertHexFingerprintsFromBin(bitArr);
