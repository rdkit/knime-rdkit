...
public class RDKitFingerprintWriterNodeModel extends AbstractRDKitNodeModel {
    ...
    protected DataTableSpec[] configure(final DataTableSpec[] inSpecs)
        ...
        boolean bIdColumnFound = SettingsUtils.autoGuessColumn(inSpecs[0], m_modelIdColumnName, StringValue.class, 0, 
		...
		File fileOutput = FileUtils.convertToFile(m_modelOutputFile.getStringValue(), false, true);
		if (fileOutput.exists()) {
            ...
            File dirOutput = fileOutput.getParentFile();
            if (dirOutput == null) {
    ...
    protected BufferedDataTable[] processing(final BufferedDataTable[] inData, InputDataInfo[][] arrInputDataInfo,
		...
		try {
			...
			for (CloseableRowIterator i = inData[0].iterator(); i.hasNext(); rowIndex++) {
				...
				// Get fingerprint
				DenseBitVector dbvFingerprint = arrInputDataInfo[0][INPUT_COLUMN_FPS].getDenseBitVector(row);
				...
				if (dbvFingerprint == null) {
					...
					int iNumBits = (int)dbvFingerprint.length();
     ...
     */
    public static String convertToFpsFormat(DenseBitVector dbvFingerprint,
    		int iForceBitLength) throws NumberFormatException {
   	
    	int iNumBits = (int)dbvFingerprint.length();

		// resultHex to store hex encoded Fingerprints
		StringBuilder sbResult = new StringBuilder(iNumBits / 4);
		...
		for (int i = 0; i < iNumBits; i++) {
			...
			if (i % 8 == 7) {
				if (iByte == 0) {
					...
					while (iByte != 0) {
						strTemp.append(HEX[(int)(iByte % 16)]);
						iByte = iByte >> 4;
					}
