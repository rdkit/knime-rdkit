...
/**
 * This class implements the node model of the RDKitKekulize node
 * providing calculations based on the open source RDKit library.
 * Kekulizes an RDKit Molecule.
 * @author Manuel Schwarze
 */
public class RDKitKekulizeNodeModel extends AbstractRDKitCalculatorNodeModel {
	...
	protected static final NodeLogger LOGGER = NodeLogger
			.getLogger(RDKitKekulizeNodeModel.class);
	...
	private final SettingsModelString m_modelInputColumnName =
			registerSettings(RDKitKekulizeNodeDialog.createInputColumnNameModel());
	...
	private final SettingsModelString m_modelNewColumnName =
			registerSettings(RDKitKekulizeNodeDialog.createNewColumnNameModel());
	...
	private final SettingsModelBoolean m_modelRemoveSourceColumns =
			registerSettings(RDKitKekulizeNodeDialog.createRemoveSourceColumnsOptionModel());
	...
	protected DataTableSpec[] configure(final DataTableSpec[] inSpecs)
		...
		SettingsUtils.autoGuessColumnName(inSpecs[0], null,
				(m_modelRemoveSourceColumns.getBooleanValue() ?
						...
						m_modelNewColumnName, strInputColumnName + " (Kekulized)");
				...
				public DataCell[] process(final InputDataInfo[] arrInputDataInfo, final DataRow row, final int iUniqueWaveId) throws Exception {
					...
					try {
						...
						try {
							...
							warnings.saveWarning(WarningConsolidator.ROW_CONTEXT.getId(), "Unable to create SMILES after kekulizing the molecule. Producing empty cell.");
							outputCell = DataType.getMissingCell();
						}
					}
					catch (final Exception exc) {
						warnings.saveWarning(WarningConsolidator.ROW_CONTEXT.getId(), "Unable to kekulize molecule. Producing empty cell.");
						outputCell = DataType.getMissingCell();
					}
