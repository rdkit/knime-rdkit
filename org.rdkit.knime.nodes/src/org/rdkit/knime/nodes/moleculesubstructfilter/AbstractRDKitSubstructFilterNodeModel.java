...
public abstract class AbstractRDKitSubstructFilterNodeModel extends AbstractRDKitNodeModel {
	 ...
	 */
	protected AbstractRDKitSubstructFilterNodeModel(final Class<? extends DataValue>... arrClassQueryType) {
		super(2, 2);
	...
	protected AbstractRDKitCellFactory createOutputFactory(final InputDataInfo[] arrInputDataInfos)
		...
		final AbstractRDKitCellFactory factory = new AbstractRDKitCellFactory(this,
			...
			public DataCell[] process(final InputDataInfo[] arrInputDataInfo, final DataRow row, final int iUniqueWaveId) throws Exception {
				...
				final List<IntCell> listFragments = new ArrayList<IntCell>();
				...
				final long iMolAtomsCount = mol.getNumAtoms();
				for (int i = 0; i < m_arrPatterns.length; i++) {
					final ROMol molPattern = m_arrPatterns[i];
					if (molPattern != null ) {
	 ...
	 */
	protected void setPreprocessingResults(final ROMol[] arrPatterns, final int iTotalEmptyPatternCells,
			final int iTotalPatternAtomsCount) {
