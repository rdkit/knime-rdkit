 ...
 * @author Greg Landrum
 */
public class RDKitMurckoScaffoldNodeModel extends NodeModel {

    private final SettingsModelString m_first =
            RDKitMurckoScaffoldNodeDialogPane.createFirstColumnModel();
        ...
        if (null == m_concate.getStringValue()) {
            if (null != m_first.getStringValue()) {
                ...
                String newName = DataTableSpec.getUniqueColumnName(inSpecs[0],
                        m_first.getStringValue() + " (Murcko)");
                m_concate.setStringValue(newName);
            } else {
                m_concate.setStringValue("Murcko Scaffold");
            }
        }
