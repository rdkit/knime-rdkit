...
public class RDKitMolecule2InChINodeModel extends AbstractRDKitCalculatorNodeModel {
    ...
    protected DataTableSpec[] configure(final DataTableSpec[] inSpecs)
        ...
        List<String> listAddColumns = new ArrayList<String>();
        List<String> listExcludeColumns = new ArrayList<String>();
        ...
        String[] arrAddColumns = listAddColumns.toArray(new String[listAddColumns.size()]);
        String[] arrExclColumns = listExcludeColumns.toArray(new String[listExcludeColumns.size()]);
        ...
        if ((m_modelExtraReturnCodeOption.getBooleanValue() || 
        	...
        	String strSuggestedName = strInputColumnName + " (InChI Extra) ";
        	String strResult = strSuggestedName;
        	
        	// Create list of all existing names
        	List<String> listNames = SettingsUtils.createMergedColumnNameList(inSpecs[0], 
	...
	protected AbstractRDKitCellFactory[] createOutputFactories(int outPort, DataTableSpec inSpec)
    	...
    	if (outPort == 0) {
    		...
    		List<DataColumnSpec> listOutputSpec = new ArrayList<DataColumnSpec>(); 
    		...
    		DataColumnSpec[] arrOutputSpec = listOutputSpec.toArray(new DataColumnSpec[listOutputSpec.size()]);
    	    ...
    	    arrOutputFactories[0] = new AbstractRDKitCellFactory(this, AbstractRDKitCellFactory.RowFailurePolicy.DeliverEmptyValues,
	   		    ...
	   		    public DataCell[] process(InputDataInfo[] arrInputDataInfo, DataRow row, int iUniqueWaveId) throws Exception {
	   		    	List<DataCell> listOutput = new ArrayList<DataCell>(3);
	   		    	
	   		    	// Calculate the new cells
	   		    	ROMol mol = markForCleanup(arrInputDataInfo[INPUT_COLUMN_MOL].getROMol(row), iUniqueWaveId);    
	   		    	ExtraInchiReturnValues extraInfo = markForCleanup(new ExtraInchiReturnValues(), iUniqueWaveId);
	   		    	String strInChICode;
	   	    		...
	   	    		if (bGenerateInChIKeys) {
	   		    		if (strInChICode == null || strInChICode.isEmpty()) {
	   		    			...
	   		    			String strInChIKey = RDKFuncs.InchiToInchiKey(strInChICode);
	   	    		...
	   	    		// Add aux info
	   	    		if (bGenerateAuxInfo) {
	   	    			String str = extraInfo.getAuxInfoPtr();
	   	    			listOutput.add(str == null ? DataType.getMissingCell() : new StringCell(str.trim()));
	   	    		}
	   	    		
	   	    		// Add message
	   	    		if (bGenerateMessage) {
	   	    			String str = extraInfo.getMessagePtr();
	   	    			listOutput.add(str == null ? DataType.getMissingCell() : new StringCell(str.trim()));
	   	    		}
	   	    		
	   	    		// Add log
	   	    		if (bGenerateLog) {
	   	    			String str = extraInfo.getLogPtr();
	   	    			listOutput.add(str == null ? DataType.getMissingCell() : new StringCell(str.trim()));
	   	    		}
