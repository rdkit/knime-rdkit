...
/**
 * This class implements the node model of the RDKitRemoveHs node
 * providing calculations based on the open source RDKit library.
 * Removes hydrogens from an RDKit Molecule.
 * @author Manuel Schwarze
 */
public class RDKitRemoveHsNodeModel extends AbstractRDKitCalculatorNodeModel {
	...
	protected static final NodeLogger LOGGER = NodeLogger
			.getLogger(RDKitRemoveHsNodeModel.class);
	...
	private final SettingsModelString m_modelInputColumnName =
			registerSettings(RDKitRemoveHsNodeDialog.createInputColumnNameModel());
	...
	private final SettingsModelString m_modelNewColumnName =
			registerSettings(RDKitRemoveHsNodeDialog.createNewColumnNameModel());
	...
	private final SettingsModelBoolean m_modelRemoveSourceColumns =
			registerSettings(RDKitRemoveHsNodeDialog.createRemoveSourceColumnsOptionModel());
	...
	protected DataTableSpec[] configure(final DataTableSpec[] inSpecs)
		...
		SettingsUtils.autoGuessColumnName(inSpecs[0], null,
				(m_modelRemoveSourceColumns.getBooleanValue() ?
						...
						m_modelNewColumnName, strInputColumnName + " (Removed Hs)");
		...
		if (outPort == 0) {
			...
			arrOutputFactories[0] = new AbstractRDKitCellFactory(this, AbstractRDKitCellFactory.RowFailurePolicy.DeliverEmptyValues,
				...
				public DataCell[] process(final InputDataInfo[] arrInputDataInfo, final DataRow row, final long lUniqueWaveId) throws Exception {
					...
					final ROMol molRemoveHs = markForCleanup(mol.removeHs(false), lUniqueWaveId);
					final RWMol molToSanitize = markForCleanup(new RWMol(molRemoveHs), lUniqueWaveId);
					RDKFuncs.sanitizeMol(molToSanitize);
					outputCell = RDKitMolCellFactory.createRDKitAdapterCell(molToSanitize);
