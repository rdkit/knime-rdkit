...
public class RDKitSaltStripperNodeModel extends AbstractRDKitCalculatorNodeModel {
	   				...
	   				// Case 2: Do the salt stripping
	   					...
	   					for (int i = 0; i < iSaltCount; i++) {
	   						...
	   						if (markForCleanup(RDKFuncs.getMolFrags(molStripping), iUniqueWaveId).size() > 1) {
	   							...
	   							ROMol molStripped = markForCleanup(RDKFuncs.deleteSubstructs(
		   				...
		   				RWMol molResult = markForCleanup(new RWMol(molStripping), iUniqueWaveId);
		   				
		   				try {
		   					RDKFuncs.sanitizeMol(molResult);
		   					outputCell = RDKitMolCellFactory.createRDKitMolCell(molResult);
		   				} 
		   				catch (Exception e) { // Sanitizing
	...
	protected void preProcessing(final BufferedDataTable[] inData, InputDataInfo[][] arrInputDataInfo,
		...
		if (hasSaltInputTable(getInputTableSpecs(inData)) && 
			...
			for (DataRow row : inData[1]) {
				ROMol molSalt = markForCleanup(arrInputDataInfo[1][INPUT_COLUMN_SALT].getROMol(row));
	...
	private List<ROMol> readSaltsFromFile() throws IOException {
		List<ROMol> listSalts = new ArrayList<ROMol>(100);
		...
		try {
			...
			while ((strLine = readerSalts.readLine()) != null) {
				...
				if (!strLine.isEmpty() && !strLine.startsWith("//") && !strLine.startsWith("'")) {
					...
					if (tokens.length > 0) {
						String strSmarts = tokens[0].trim();
	 ...
	 */
	public static boolean hasSaltInputTable(PortObjectSpec[] inSpecs) {
