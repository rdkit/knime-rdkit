...
public class RDKitSubstructFilterNodeModel extends AbstractRDKitNodeModel {
    ...
    protected DataTableSpec[] configure(final DataTableSpec[] inSpecs)
        ...
        ROMol pattern = markForCleanup(RWMol.MolFromSmarts(m_modelSmartsQuery.getStringValue(), 0, true));
        if (pattern == null) {
		...
		AbstractRDKitCellFactory factory = new AbstractRDKitCellFactory(this, 
   		    ...
   		    public DataCell[] process(InputDataInfo[] arrInputDataInfo, DataRow row, int iUniqueWaveId) throws Exception {
				...
				// Get a thread-save instance of the pattern    		    	
   		    	ROMol molPattern = m_pattern.get();
   		    	...
   		    	if (mol != null && molPattern != null && 
   		    		...
   		    		boolean matched = mol.hasSubstructMatch(molPattern);
	                
   		    		// We found a match
   		    		if (matched) {  		    			
   		    			
   		    			MatchHandling matchHandling = m_modelMatchHandling.getValue();
   		    			...
   		    			// Deliver more details about the substructure match
   		                    ...
   		                    try {   
	   		    				ArrayList<DataCell> listCellAtomList = new ArrayList<DataCell>();
	                        	Match_Vect_Vect vecVecMatches = mol.getSubstructMatches(molPattern);
	                        	StringBuilder sb = new StringBuilder("( ");
	                        	
	                        	if (vecVecMatches != null) {
	                        		int iCountMatches = (int)vecVecMatches.size();
	                        		for (int matchIndex = 0; matchIndex < iCountMatches; matchIndex++) {
	                        			...
	                        			Match_Vect vecMatch = vecVecMatches.get(matchIndex);
	                        			int iCountMatchingAtoms = (int)vecMatch.size();
		                        		for (int atomIndex = 0; atomIndex < iCountMatchingAtoms; atomIndex++) {
		                        			Int_Pair pair = vecMatch.get(atomIndex);
