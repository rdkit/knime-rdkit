...
public class AdditionalHeaderInfoRenderer extends ColumnHeaderRenderer {
	...
	public synchronized Component getTableCellRendererComponent(
		...
		if (value instanceof DataColumnSpec) {
			...
			if (arrHandlers != null && arrHandlers.length > 0) {
				// Change background
				setBackground(changeColor(getBackground(), 10));
				...
				setIcon(new VerticalCompoundIcon(iconAddInfo, iconHeader, RIGHT, 0));
			}
		}
	 ...
	 */
	protected Icon createTextIcon(final int xCoord, final int yCoord, final int width, final int height) {
	 ...
	 */
	protected Icon createIconWrapper(final JTable table, final int column,
	 ...
	 *         was <code>null</code>.
	 */
	protected Color changeColor(final Color color, final int difference) {
		Color retColor = null;
