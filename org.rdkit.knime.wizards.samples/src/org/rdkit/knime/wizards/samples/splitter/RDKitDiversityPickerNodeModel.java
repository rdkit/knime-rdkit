 ...
 * ---------------------------------------------------------------------
 */
package org.rdkit.knime.wizards.samples.splitter;
...
public class RDKitDiversityPickerNodeModel extends AbstractRDKitSplitterNodeModel {
	...
	protected void preProcessing(final BufferedDataTable[] inData,
		...
		final long lRowCount = (int)inData[0].size();
		...
		while (it.hasNext()) {
			...
			if (iRowIndex % 20 == 0) {
				AbstractRDKitNodeModel.reportProgress(subExecReadingFingerprints, iRowIndex, lRowCount, row, " - Reading fingerprints");
			}
		...
		final Int_Vect intVector = markForCleanup(RDKFuncs.pickUsingFingerprints(vFingerprints, m_modelNumberToPick.getIntValue()));
		...
		m_ebvRowsToKeep = markForCleanup(new ExplicitBitVect(lRowCount));
